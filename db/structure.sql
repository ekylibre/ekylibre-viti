...
CREATE FUNCTION lexicon.deny_changes() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN RAISE EXCEPTION '% denied on % (master data)', TG_OP, TG_RELNAME; END; $$;
...
CREATE UNLOGGED TABLE lexicon.ephy_cropsets (
    id character varying NOT NULL,
...
CREATE UNLOGGED TABLE lexicon.registered_postal_zones (
    country character varying NOT NULL,
    code character varying NOT NULL,
...
CREATE VIEW public.formatted_cvi_cadastral_plants AS
 SELECT cvi_cadastral_plants.id,
   ...
   FROM (((((public.cvi_cadastral_plants
     ...
     LEFT JOIN lexicon.registered_postal_zones ON (((locations.insee_number)::text = (registered_postal_zones.code)::text)))
...
CREATE INDEX registered_crop_zones_centroid_idx ON lexicon.registered_crop_zones USING gist (centroid);
...
CREATE OR REPLACE VIEW public.formatted_cvi_land_parcels AS
 SELECT cvi_land_parcels.id,
   ...
   FROM ((((((public.cvi_land_parcels
     ...
     LEFT JOIN lexicon.master_vine_varieties rootstocks ON (((land_parcel_rootstocks.rootstock_id)::text = (rootstocks.id)::text)))
     LEFT JOIN lexicon.registered_postal_zones ON (((locations.insee_number)::text = (registered_postal_zones.code)::text)))
...
--
-- Name: ephy_cropsets deny_changes; Type: TRIGGER; Schema: lexicon; Owner: -
--

CREATE TRIGGER deny_changes BEFORE INSERT OR DELETE OR UPDATE OR TRUNCATE ON lexicon.ephy_cropsets FOR EACH STATEMENT EXECUTE PROCEDURE lexicon.deny_changes();
