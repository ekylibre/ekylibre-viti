...
module Ekylibre
  class CviJsonExchanger < CviExchanger
    ...
    def import_cvi_cadastral_plants(h_cvi_statement)
      ...
      designation_of_origin = RegistredProtectedDesignationOfOrigin.where("geographic_area = ?", h_cvi_statement[:product]).first
      ...
      vine_variety = MasterVineVariety.where(specie_name: h_cvi_statement[:grape_variety], category_name:"Cépage" ).first
      unless vine_variety
      ...
      rootstock = MasterVineVariety.where(customs_code: h_cvi_statement[:rootstock] , category_name: "Porte-greffe").first
      unless rootstock
      ...
      CviCadastralPlant.create!(
        h_cvi_statement.to_h.select { |key, _| CVI_CADASTRAL_PLANT_KEYS.include? key }
          .merge(cvi_statement_id: cvi_statement.id, land_parcel_id: cadastral_land_parcel_zone.id,designation_of_origin_id: designation_of_origin.id, vine_variety_id: vine_variety.id, rootstock_id: rootstock.id, measure_value_value: h_cvi_statement[:area], measure_value_unit:"hectare")
      )
    end
    ...
    def import_cvi_statements(h_cvi_statement)
      cvi_statement = CviStatement.find_by(cvi_number: h_cvi_statement[:cvi_number])
      if cvi_statement
        total_area = cvi_statement.measure_value_value + h_cvi_statement[:area]
        ...
        CviStatement.create!(
          h_cvi_statement.to_h.select { |key, _| CVI_STATEMENT_KEYS.include? key }.merge(cadastral_sub_plant_count: 1, cadastral_plant_count: 1, measure_value_value: h_cvi_statement[:total_area], measure_value_unit:"hectare")
        )
      end
    end
