module Backend
  class CviStatementsController < Backend::BaseController
    ...
    def self.cvi_cadastral_plants_conditions
      ...
      code << "c[0] << ' AND #{CviCadastralPlant.table_name}.measure_value_value BETWEEN ? AND ?'\n"
      code << "c << lower\n"
      code << "c << higher\n"
      code << "end\n"
      ...
      #inter_row_distance
      code << "if params[:inter_row_distance] \n"
      code << "lower,higher = params[:inter_row_distance].split(',').map(&:to_i)\n"
      code << "c[0] << ' AND #{CviCadastralPlant.table_name}.inter_row_distance BETWEEN ? AND ?'\n"
      code << "c << lower\n"
      code << "c << higher\n"
      code << "end\n"

      #inter_vine_plant_distance
      code << "if params[:inter_vine_plant_distance]\n"
      code << "lower,higher = params[:inter_row_distance].split(',').map(&:to_i)\n"
      code << "c[0] << ' AND #{CviCadastralPlant.table_name}.inter_vine_plant_distance BETWEEN ? AND ?'\n"
      code << "c << lower\n"
      code << "c << higher\n"
      code << "end\n"
