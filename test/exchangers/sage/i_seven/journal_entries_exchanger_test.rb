  ...
  module ISeven
    class JournalEntriesExchangerTest < ActiveExchanger::TestCase
      test 'import' do
        ...
        assert_equal 4364.8, item1.where(account_id: Account.where(number: '707100000').pluck(:id)).first.real_credit.to_f
        assert_equal 4363.49, item1.where(account_id: Account.where(number: '411008162').pluck(:id)).first.real_debit.to_f
        assert_equal 1.06, item1.where(account_id: Account.where(number: '637800000').pluck(:id)).first.real_debit.to_f
        assert_equal 0.25, item1.where(account_id: Account.where(number: '637801000').pluck(:id)).first.real_debit.to_f

        assert_equal 4, journal_entry_1.items.count
        assert_equal 2, journal_entry_2.items.count
        assert_equal 282.18, item2.where(account_id: Account.where(number: '401000483').pluck(:id)).first.real_debit.to_f
        assert_equal 282.18, item2.where(account_id: Account.where(number: '512300000').pluck(:id)).first.real_credit.to_f
