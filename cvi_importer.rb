...
class CVIImporter
  def initialize(csv_file_path = './Modele_CVI_9sep19.csv')
    @csv_file_path = csv_file_path
  end

  def import
    CSV.foreach(csv_file_path, headers: true, header_converters: ->(h) { if HEADER_CONVERSION[h]; HEADER_CONVERSION[h] else raise "Unknown column name #{h}" end }) do |row|
      convert_types(row)
